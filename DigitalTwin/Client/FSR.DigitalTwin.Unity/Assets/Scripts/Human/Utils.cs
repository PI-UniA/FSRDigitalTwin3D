...
public class Utils
{
    ...
    static Keypoint[] DecodePose(Keypoint root, Tensor scores, Tensor offsets, int stride, Tensor displacementsFwd, Tensor displacementsBwd)
    {
        ...
        for (int edge = numEdges - 1; edge >= 0; --edge)
        {
                ...
                instanceKeypoints[targetKeypointId].score == 0.0f)
            {
                instanceKeypoints[targetKeypointId] = TraverseToTargetKeypoint(
                    edge, instanceKeypoints[sourceKeypointId], targetKeypointId, scores, offsets, stride, displacementsBwd);
            }
        }
        ...
        for (int edge = 0; edge < numEdges; ++edge)
        {
            ...
            {
                instanceKeypoints[targetKeypointId] = TraverseToTargetKeypoint(
                    edge, instanceKeypoints[sourceKeypointId], targetKeypointId, scores, offsets, stride, displacementsFwd);
            }
        }
    ...
    static bool ScoreIsMaximumInLocalWindow(int keypointId, float score, int heatmapY, int heatmapX, int localMaximumRadius, Tensor heatmaps)
    {
        ...
        for (int yCurrent = yStart; yCurrent < yEnd; ++yCurrent)
        {
            ...
            {
                // Check if the score for at the current heatmap location is the highest within the specified radius
    ...
    // Iterate through the heatmaps and create a list of indicies with the highest values within the provided radius.
        ...
        float scoreThreshold = 0.5f, int nmsRadius = 20)
    {
        ...
        while (poses.Count < maxPoseDetections && list.Count > 0)
        {
            ...
            Keypoint[] keypoints = DecodePose(root, heatmaps, offsets, stride, displacementsFwd, displacementBwd);
